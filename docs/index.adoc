...
== Selecting columns (select) and aliases

Use the select query parameter to return only specific columns. You can also assign aliases to the output field names.
...
- If an alias is not provided, the column name is used as the output key.
